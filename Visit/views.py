...
class view_transportViewset(viewsets.ModelViewSet):
    # queryset = PageView.objects.all().order_by('-count')[:10]
    queryset = view_transport.objects.all().order_by('-count')
    # permission_classes = IsAuthenticatedOrReadOnly,
    # authentication_classes= JWTAuthentication, SessionAuthentication
    serializer_class = view_transportSerializer
    filter_backends = [DjangoFilterBackend, filters.SearchFilter]
    filterset_fields = ['identification']
    search_fields = ['identification']
    ...
    def create(self, request):
        ...
        # Créer un nouvel objet PageView avec les autres champs
        view_transport_instance, created = view_transport.objects.get_or_create(identification=data['identification'])
